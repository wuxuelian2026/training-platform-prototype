    ...
    'crm/classes': {
      groups: [
        { heading: '发布班级字段', fields: [
          ...
          // CR-2026-012：教学属性只读带入；运营四字段写入课程档案（课程级存储），不写班级记录。
      ...
      notes: [
        ...
        '报名截止时间之后不再接受新的报名或支付。',
        '课程封面、图文详情、课程标签与 C 端推荐语写入课程档案（课程级存储），不写入班级记录；字段定义与发布商品共用同一份规格。',
        '完整课程首次发布必须上传课程封面；轻量课程档案的课程封面可选填。同一课程二次发布时运营四字段只读带入，可通过“修改展示信息”入口按课程级覆盖。'
      ]
    },
    'crm/leads': {
      groups: [
