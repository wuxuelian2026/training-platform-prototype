    ...
    'mall/products': {
      groups: [
        { heading: '发布商品字段', fields: [
          ...
          // CR-2026-012：教学属性只读带入；运营四字段写入课程档案（课程级存储），不写商品记录。
      ...
      notes: [
        ...
        '价格、试看与上下架以商品配置为准，课程档案不得覆盖售卖字段。',
        '关闭试看时未购买账号不能播放任何课时；开启试看时只能播放第一课时。',
        '课程封面、图文详情、课程标签与 C 端推荐语写入课程档案（课程级存储），不写入商品记录；字段定义与发布班级共用同一份规格。',
        '完整课程首次发布必须上传课程封面；同一课程二次发布时运营四字段只读带入，可通过“修改展示信息”入口按课程级覆盖，影响该课程所有展示位。'
      ]
    }
  }
};
