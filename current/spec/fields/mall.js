    ...
    'mall/products': {
      groups: [
        { heading: '发布商品字段', fields: [
          ...
          { id: 'FD-MALL-058', label: '是否推荐', type: '开关', length: '是 / 否', required: '否', note: '仅已上架且关联课程启用时生效；控制商品是否进入学员端首页“视频课程推荐”，不改变上下架状态', constraints: { default: false } },
      ...
      notes: [
        ...
        '是否推荐是独立展示属性：关闭后仅移出学员端首页推荐栏目；商品列表、课程列表、订单和学习权限不受影响。商品下架或关联课程停用时首页自动不展示，但保留推荐值。',
        '商品保存后为草稿，从列表手动上架或下架；上架前必须上传封面。同一商品再次编辑时展示四字段只读带入，点“修改展示信息”后可编辑。',
