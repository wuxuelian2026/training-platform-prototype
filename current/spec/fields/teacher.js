...
export const TEACHER_APP_FIELD_SPEC = {
  ...
  pages: {
    ...
    'teacher/application-create': {
      ...
      notes: [
        '课程申报不提供保存草稿，提交即进入审核中；驳回或撤销后重新提交沿用原申报编号。',
    ...
    'teacher/applications': {
      groups: [
        { heading: '申报列表操作字段', fields: [
          { id: 'FD-TAPP-019', label: '撤销申报', type: '按钮', length: '—', required: '是', note: '仅“审核中”的申报可撤销；撤销后可修改并沿用原申报编号重新提交', constraints: { action: true } },
          { id: 'FD-TAPP-020', label: '撤销确认', type: '单选', length: '确认撤销 / 取消', required: '是', note: '二次确认，避免误撤销已提交的申报', constraints: { options: ['确认撤销', '取消'] } }
        ] }
      ],
      notes: [
        '发起申报不保存草稿，提交后直接进入“审核中”，取消或离开页面不产生记录。',
        '“审核中”的申报不可直接修改，只能撤销；已驳回或已撤销的申报可编辑并沿用原申报编号重新提交。',
