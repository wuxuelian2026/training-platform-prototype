...
// I1-DEMO-01：课程按大类集中归属（舞蹈→王玥、美术→李青、音乐→林悦），便于切换演示账号时口径一致。
const applyDemoOwner = (item) => {
  const owner = demoOwnerForMajor(item.major, item.teacher);
  if (!owner || owner === item.teacher) return item;
  const account = teacherAccounts.find((teacher) => teacher.name === owner);
  if (!account) return item;
  return {
    ...item,
    teacherId: account.id,
    teacher: account.name,
    teacherNo: account.no,
    teacherUnit: account.unit,
    teacherTitle: account.title,
    teacherProfessional: account.professional,
    teacherInfo: `${account.name} · ${account.title} · ${account.professional} · ${account.unit}`
  };
};
