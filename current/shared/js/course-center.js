...
import { demoId, demoTime, dictionaryDefault, dictionaryItems, fileSpecSettings, readDemoState, removeDemoRecord, updateDemoRecord, upsertDemoRecord } from './demo-store.js';
...
import { defaultLessonDuration, lessonDurationValues } from './timetable-settings.js';
...
// CR-2026-103：难度等级、适合年龄与适用等级读取「系统管理 → 数据字典」，页面不再写死选项。
const courseDifficultyOptions = () => dictionaryItems('course_difficulty');
const courseAgeOptions = () => dictionaryItems('suitable_age');
const courseDefaultDifficulty = () => dictionaryDefault('course_difficulty') || courseDifficultyOptions()[0] || '';
...
const catalog = {
  ...
  groupSort: { '音乐类': 1, '舞蹈类': 2, '美术类': 3, '戏剧类': 4 },
  groupIcon: {},
  categories: [
...
const modal = (title, subtitle, content, options = {}) => {
  ...
  dialog.className = `course-modal${options.large ? ' large' : ''}${options.drawer ? ' drawer' : ''}`;
...
// 否则编辑既有课程时当前值会从下拉里消失。
// 停用沿层级生效：专业自身、所属分类或所属门类停用后都不再出现在新的申报与筛选项；
// 已选中的值保留，否则编辑既有记录时当前值会从下拉里消失。
const professionalFilter = (name, selected = '') => `<select name="${name}"><option value="">全部专业</option>${catalog.majors.filter((item) => item.name === selected || (item.status === '启用' && (catalog.categories.find((category) => category.name === item.parent)?.status || '启用') === '启用' && (catalog.groupStatus[item.group] || '启用') === '启用')).map(item => option(item.name, item.name === selected)).join('')}</select>`;
...
function renderContent(page) {
  ...
  page.innerHTML = `<div class="course-page">${pageShell('课程内容编排', '', '<button class="button primary" type="button" data-action="new-direct-course">新增面授课程</button>')}<section class="course-surface"><form class="course-filter" data-form="content-filter"><div class="course-filter-head"><strong>筛选条件</strong></div><div class="course-filter-grid">${filterField('编排状态', selectWithValues('status', ['待编排', '编排中'], state.contentFilters.status || '', '全部状态'))}${filterField('课程类型', selectWithValues('type', ['视频课程', '面授课程'], state.contentFilters.type || '', '全部类型'))}${filterField('所属专业', professionalFilter('major', state.contentFilters.major))}${filterField('关键词', `<input name="keyword" value="${escapeHtml(state.contentFilters.keyword || '')}" placeholder="课程名称或教师姓名" />`)}</div><div class="course-filter-actions"><button class="button" type="reset">重置</button><button class="button primary" type="submit">查询</button></div></form><div class="course-table-head"><div><strong>内容编排中的课程</strong><span> 共 ${filtered.length} 条，按最近编辑时间排序</span></div></div><div class="course-table-wrap">${filtered.length ? `<table><thead><tr><th>课程名称</th><th>课程来源</th><th>课程类型</th><th>所属专业</th><th>申报教师</th><th>总课时</th><th>编排状态</th><th>上次编辑时间</th><th>操作</th></tr></thead><tbody>${filtered.map(item => `<tr><td><span class="primary-cell">${escapeHtml(item.name)}</span><span class="sub-cell">${item.id}</span></td><td>${escapeHtml(item.source || (item.applicationId ? '教师申报' : '后台新增'))}</td><td>${escapeHtml(item.type)}</td><td>${escapeHtml(item.major)}</td><td>${escapeHtml(item.teacher)}</td><td>${hoursText(item.hours, item.status === '已完成')}</td><td>${tag(item.status)}</td><td>${escapeHtml(item.updatedAt)}</td><td><div class="course-actions">${button(item.status === '待编排' ? '开始编排' : '继续编排', 'content-workbench', `data-id="${item.id}"`)}</div></td></tr>`).join('')}</tbody></table>` : '<div class="course-empty"><strong>暂无内容编排中的课程</strong><span>教师申报通过或后台新增的面授课程会进入此处。</span></div>'}</div>${pagination(filtered.length, '门课程')}</section></div>`;
...
function renderCatalog(page) {
  const groups = Object.keys(professionalTree).sort((a, b) => (catalog.groupSort[a] ?? 999) - (catalog.groupSort[b] ?? 999) || a.localeCompare(b, 'zh-CN'));
  ...
  const groupNodes = groups.map((group) => {
    const categories = catalog.categories.filter((item) => item.parent === group).sort((a, b) => (a.sort ?? 999) - (b.sort ?? 999) || a.name.localeCompare(b.name, 'zh-CN'));
    const groupMajors = catalog.majors.filter((item) => item.group === group);
    const categoryNodes = categories.map((category) => {
      const majors = catalog.majors.filter((item) => item.parent === category.name && item.group === group).sort((a, b) => (a.sort ?? 999) - (b.sort ?? 999) || a.name.localeCompare(b.name, 'zh-CN'));
      ...
      return `<li class="catalog-tree-node catalog-tree-category"><div class="catalog-tree-row">${caret(categoryKey, majors.length > 0)}<button type="button" class="catalog-tree-copy catalog-tree-button${state.selectedCategory === category.name ? ' is-selected' : ''}" data-action="catalog-select-category" data-value="${escapeHtml(category.name)}"><strong>${escapeHtml(category.name)}</strong><small>${majors.length} 个专业</small></button>${nodeActions('category', category)}</div>${majors.length ? `<ul class="catalog-tree-children"${isOpen(categoryKey) ? '' : ' hidden'}>${majorNodes}</ul>` : ''}</li>`;
    }).join('');
    const groupKey = `group:${group}`;
    return `<li class="catalog-tree-node catalog-tree-group"><div class="catalog-tree-row">${caret(groupKey, categories.length > 0)}<button type="button" class="catalog-tree-copy catalog-tree-button${state.selectedGroup === group ? ' is-selected' : ''}" data-action="catalog-select-group" data-value="${escapeHtml(group)}"><strong>${escapeHtml(group)}</strong><small>${categories.length} 个分类 · ${groupMajors.length} 个专业</small></button>${nodeActions('group', group)}</div>${categories.length ? `<ul class="catalog-tree-children"${isOpen(groupKey) ? '' : ' hidden'}>${categoryNodes}</ul>` : ''}</li>`;
  }).join('');
...
function renderApplicationReview(page) {
  ...
  // CR-2026-084：重提后不再展示上一轮审核意见；待审核（含驳回后重提）一律按“暂无审核意见”渲染，避免遗留数据穿透。
  const showReview = Boolean(item.review) && !pending;
  const lastReview = showReview
    ? `<section class="course-detail-section wide"><h3>审核意见</h3><div class="course-review-box">${tag(item.status)}<p>${escapeHtml(item.review)}</p><small>审批人：${escapeHtml(item.reviewedBy || '—')} · 审批时间：${escapeHtml(item.reviewedAt || '—')}</small></div></section>`
    : '<section class="course-detail-section wide"><h3>审核意见</h3><p class="course-hint">暂无审核意见。仅展示本次审核结果，不提供历史意见时间线。</p></section>';
  ...
  dialog.addEventListener('click', event => {
    ...
    if (action === 'delete-chapter') {
      ...
      confirmAction({
        ...
        detail: '删除即时写入编排草稿，完成编排后才进入全部课程。', confirmLabel: '确认删除',
    ...
    if (action === 'add-lesson') { paneView = 'lesson'; paneError = ''; lessonDraft = { index: null, name: '', order: (item.chapters[activeChapter]?.lessons.length || 0) + 1, target: '', duration: defaultLessonDuration(), kind: '理论', description: '' }; renderWorkbench(); }
    ...
    if (action === 'delete-lesson') {
      ...
      confirmAction({
        title: '删除课时', message: '确认删除该课时？', detail: '删除即时写入编排草稿，完成编排后才进入全部课程。', confirmLabel: '确认删除',
...
function openLessonForm(course, chapterIndex, lessonIndex, onDone) {
  const current = lessonIndex === null ? { name: '', target: '', duration: defaultLessonDuration(), kind: '理论', description: '', resources: [] } : course.chapters[chapterIndex].lessons[lessonIndex];
...
// 单个资源文件上限 100GB；展示时按大小自动切换 GB / MB。
const RESOURCE_SIZE_LIMIT_BYTES = 100 * 1024 * 1024 * 1024;
const formatResourceSize = (bytes) => { const mb = Number(bytes) / 1024 / 1024; return mb >= 1024 ? `${(mb / 1024).toFixed(2)}GB` : `${mb.toFixed(1)}MB`; };
function openResourceForm(id = null) {
  const current = id ? resources.find(item => item.id === id) : { name: '', type: '教学视频', major: '', level: '启蒙', size: '', teacher: readAdminSession()?.name || '李教务', uploadedAt: '刚刚', references: 0, ext: 'video', fileName: '' };
  const dialog = modal(id ? '编辑资源' : '上传资源', `支持 MP4、PPT/PDF、MP3、JPG/PNG；单个文件≤${fileSpecSettings().resourceGb}GB`, `<form data-form="resource-form" data-id="${id || ''}"><div class="course-detail-grid" style="margin-top:18px"><div class="course-field"><label>资源名称 <span class="sub-cell">必填</span></label><input name="name" required value="${escapeHtml(current.name)}" placeholder="填写资源名称" /></div><div class="course-field"><label>资源类型 <span class="sub-cell">必填</span></label>${selectWithValues('type', ['教学视频', '课件PPT', '乐谱PDF', '音频示范', '其他'], current.type, '')}</div><div class="course-field"><label>所属专业 <span class="sub-cell">必填</span></label>${professionalFilter('major', current.major)}</div><div class="course-field"><label>适用等级 <span class="sub-cell">必填</span></label>${selectWithValues('level', courseDifficultyOptions(), current.level, '')}</div><div class="course-upload wide"><label for="resource-file"><strong>资源文件 <span class="sub-cell">必填</span></strong></label><input id="resource-file" name="file" type="file" accept=".mp4,.ppt,.pptx,.pdf,.mp3,.jpg,.jpeg,.png" ${id ? '' : 'required'} /><small data-file-note>${escapeHtml(current.fileName || current.name || '请选择需要上传的文件')}</small></div><div class="course-field wide"><label>资源简介</label><textarea name="intro" placeholder="填写资源简介">${escapeHtml(current.intro || '')}</textarea></div><div class="course-field wide"><label>标签</label><input name="tags" value="${escapeHtml(current.tags || '')}" placeholder="多个标签用逗号分隔" /></div></div><div class="course-modal-actions"><button class="button" type="button" data-action="close-modal">取消</button><button class="button primary" type="submit">${id ? '保存修改' : '确认上传'}</button></div></form>`);
  dialog.querySelector('[name=file]').addEventListener('change', event => { const file = event.target.files[0]; if (file) dialog.querySelector('[data-file-note]').textContent = `${file.name} · ${formatResourceSize(file.size)}`; });
  dialog.querySelector('form').addEventListener('submit', event => { event.preventDefault(); const form = event.target; const data = new FormData(form); const file = form.querySelector('[name=file]').files[0]; const type = data.get('type'); if (!data.get('name').trim() || !data.get('major') || !data.get('level')) { showToast('请补齐资源名称、专业和适用等级', 'error'); return; } if (!id && !file) { showToast('请选择资源文件', 'error'); return; } if (file) { if (file.size > RESOURCE_SIZE_LIMIT_BYTES) { showToast(`文件超过${RESOURCE_SIZE_LIMIT_BYTES / 1024 / 1024 / 1024}GB限制，请重新选择`, 'error'); return; } } const record = { id: id || `res-${Date.now()}`, name: data.get('name').trim(), type, major: data.get('major'), level: data.get('level'), size: file ? formatResourceSize(file.size) : current.size, teacher: current.teacher, uploadedAt: current.uploadedAt, references: current.references, ext: type === '教学视频' ? 'video' : type === '音频示范' ? 'audio' : type === '其他' ? 'image' : 'document', intro: data.get('intro').trim(), tags: data.get('tags').trim() }; if (id) { Object.assign(current, record); persistResource(record); } else { resources.unshift(record); persistResource(record); } closeModal(); renderResources(root()); showToast(id ? '资源信息已保存' : '资源上传成功'); });
}
...
// 排序允许 0 及以上的整数（规格 FD-COURSE-003），因此不能用 || 兜底，否则 0 会被当成未填。
const catalogSortValue = (raw, fallback) => { const parsed = Number(raw); return Number.isFinite(parsed) && parsed >= 0 ? parsed : fallback; };
// 目录新增与编辑统一在一个抽屉页完成：三个入口只是预置不同层级，新增时可在抽屉内
// 切换门类／分类／专业，上级节点随层级联动；编辑既有目录时层级只读。
function openCatalogForm(type, id = null) {
  const LEVEL_LABELS = { group: '门类', category: '分类', major: '专业' };
  const LEVEL_BY_LABEL = { 门类: 'group', 分类: 'category', 专业: 'major' };
  const source = type === 'group'
    ? { name: id || '', sort: catalog.groupSort[id] ?? 1, icon: catalog.groupIcon[id] || '' }
    : type === 'category'
      ? catalog.categories.find(item => item.id === id) || { name: '', parent: state.selectedGroup, sort: catalog.categories.length + 1 }
      : catalog.majors.find(item => item.id === id) || { name: '', parent: state.selectedCategory, group: state.selectedGroup, sort: catalog.majors.length + 1 };
  let level = type;
  const draft = { name: source.name || '', sort: source.sort ?? 1, icon: source.icon || '', parent: source.parent || '' };
  const dialog = modal('', '名称为必填项，目录将供全系统级联选择使用', '<div data-catalog-form-root></div>', { drawer: true });
  const holder = dialog.querySelector('[data-catalog-form-root]');
  const titleOf = () => `${id ? '编辑' : '新增'}${LEVEL_LABELS[level]}`;
  const parentOptionsFor = (target) => target === 'category' ? Object.keys(professionalTree) : target === 'major' ? catalog.categories.map((item) => item.name) : [];
  const levelField = () => id
    ? `<div class="course-field"><label>目录层级</label><input class="readonly-field" value="${LEVEL_LABELS[level]}" readonly /></div>`
    : `<div class="course-field"><label>目录层级 <span class="sub-cell">必填</span></label><select name="level">${Object.keys(LEVEL_BY_LABEL).map((label) => option(label, label === LEVEL_LABELS[level])).join('')}</select></div>`;
  const parentField = () => level === 'group'
    ? `<div class="course-field"><label>图标</label><input name="icon" maxlength="20" value="${escapeHtml(draft.icon)}" placeholder="可选图标名称" /></div>`
    : `<div class="course-field"><label>上级节点 <span class="sub-cell">必填</span></label><select name="parent">${parentOptionsFor(level).map((value) => option(value, value === draft.parent)).join('')}</select></div>`;
  const capture = () => {
    const form = holder.querySelector('form');
    if (!form) return;
    const data = new FormData(form);
    if (data.get('name') !== null) draft.name = String(data.get('name'));
    if (data.get('sort') !== null) draft.sort = data.get('sort');
    if (data.get('icon') !== null) draft.icon = String(data.get('icon'));
    if (data.get('parent') !== null) draft.parent = String(data.get('parent'));
  };
  const render = () => {
    holder.innerHTML = `<form data-form="catalog-form" data-type="${level}" data-id="${id || ''}"><div class="course-detail-grid">${levelField()}<div class="course-field"><label>名称 <span class="sub-cell">必填</span></label><input name="name" required value="${escapeHtml(draft.name)}" placeholder="填写名称" /></div>${parentField()}<div class="course-field"><label>排序</label><input name="sort" type="number" min="0" value="${draft.sort ?? 1}" /></div></div><div class="course-modal-actions"><button class="button" type="button" data-action="close-modal">取消</button><button class="button primary" type="submit">保存目录</button></div></form>`;
    const heading = dialog.querySelector('.course-modal-header h2');
    if (heading) heading.textContent = titleOf();
  };
  holder.addEventListener('change', (event) => {
    const next = event.target.closest('[name=level]');
    if (!next) return;
    capture();
    level = LEVEL_BY_LABEL[next.value] || level;
    const options = parentOptionsFor(level);
    if (!options.includes(draft.parent)) draft.parent = level === 'major' ? (state.selectedCategory || options[0] || '') : (state.selectedGroup || options[0] || '');
    render();
  });
  holder.addEventListener('submit', (event) => {
    ...
    const name = String(data.get('name') || '').trim();
    if (!name) return;
    if (level === 'group') { const oldName = id || ''; if (oldName && oldName !== name && professionalTree[oldName]) { professionalTree[name] = professionalTree[oldName]; delete professionalTree[oldName]; catalog.groupStatus[name] = catalog.groupStatus[oldName] || '启用'; delete catalog.groupStatus[oldName]; catalog.groupSort[name] = catalog.groupSort[oldName]; delete catalog.groupSort[oldName]; catalog.groupIcon[name] = catalog.groupIcon[oldName]; delete catalog.groupIcon[oldName]; catalog.categories.filter(item => item.parent === oldName).forEach(item => { item.parent = name; }); catalog.majors.filter(item => item.group === oldName).forEach(item => { item.group = name; }); } if (!professionalTree[name]) professionalTree[name] = {}; catalog.groupStatus[name] ||= '启用'; catalog.groupSort[name] = catalogSortValue(data.get('sort'), catalog.groupSort[name] ?? 1); catalog.groupIcon[name] = String(data.get('icon') || '').trim(); state.selectedGroup = name; }
    if (level === 'category') { const record = id ? catalog.categories.find(item => item.id === id) : { id: `cat-${Date.now()}` }; Object.assign(record, { name, parent: data.get('parent'), sort: catalogSortValue(data.get('sort'), record.sort ?? (catalog.categories.length + 1)), status: record.status || '启用' }); if (!id) catalog.categories.push(record); state.selectedGroup = record.parent; state.selectedCategory = record.name; }
    if (level === 'major') { const record = id ? catalog.majors.find(item => item.id === id) : { id: `major-${Date.now()}`, teachers: 0, courses: 0, resources: 0 }; const parent = data.get('parent'); const category = catalog.categories.find(item => item.name === parent); Object.assign(record, { name, parent, group: category?.parent || state.selectedGroup, sort: catalogSortValue(data.get('sort'), record.sort ?? (catalog.majors.length + 1)), status: record.status || '启用' }); if (!id) catalog.majors.push(record); state.selectedGroup = record.group; state.selectedCategory = parent; }
    closeModal();
    renderCatalog(root());
    showToast('目录已保存');
  });
