...
const library = [
  { id: 'LIB-001', name: '声乐演唱技巧', archive: '完整课程', type: '视频课程', major: '声乐演唱', teacher: '陈晨', hours: 12, status: '已完成', cover: '已配置', difficulty: '中级', ages: ['青少年', '成人'], detail: '围绕气息、共鸣、咬字与作品处理，建立完整演唱方法。', tags: ['声乐', '发声'], recommendation: '跟着示范练习，建立稳定发声习惯。' },
  { id: 'LIB-002', name: '舞蹈基本功', archive: '完整课程', type: '面授课程', major: '中国舞', teacher: '王玥', hours: 16, status: '已完成', cover: '已配置', difficulty: '初级', ages: ['少儿'], detail: '从身体控制、节奏训练到基本舞姿，建立少儿中国舞的基础训练体系。', tags: ['中国舞', '基础'], recommendation: '从每一次站立开始建立身体控制。' },
  { id: 'LIB-003', name: '少儿美术兴趣班', archive: '轻量课程档案', type: '面授课程', major: '少儿绘画', teacher: '李青', hours: 20, status: '不适用', cover: '未配置', difficulty: '启蒙', ages: ['少儿'], detail: '以主题创作和材料体验激发少儿绘画兴趣。', tags: ['美术', '少儿'], recommendation: '让孩子在创作中发现自己的表达方式。' },
  { id: 'LIB-004', name: '朗诵与主持基础', archive: '轻量课程档案', type: '面授课程', major: '朗诵与主持', teacher: '赵可', hours: 16, status: '不适用', cover: '未配置', difficulty: '初级', ages: ['青少年'], detail: '训练普通话、气息和舞台表达，适合青少年入门。', tags: ['戏剧', '表达'], recommendation: '用声音和表情讲好每一个故事。' },
  { id: 'LIB-005', name: '古筝入门体验课', archive: '轻量课程档案', type: '面授课程', major: '古筝', teacher: '周宁', hours: 8, status: '不适用', cover: '未配置', difficulty: '启蒙', ages: ['少儿', '成人'], detail: '通过基础指法和短曲体验，帮助学员认识古筝。', tags: ['古筝', '体验'], recommendation: '一节课认识古筝，也认识音乐的乐趣。' }
];
...
function openWorkbench(id) {
  ...
  dialog.addEventListener('click', event => {
    ...
    if (action === 'add-chapter' || action === 'edit-chapter') openChapterForm(item, action === 'edit-chapter' ? Number(target.dataset.index) : null, renderWorkbench);
    if (action === 'delete-chapter') { if (window.confirm('删除章节会同时删除章节下的所有课时，确认继续？')) { item.chapters.splice(Number(target.dataset.index), 1); activeChapter = Math.max(0, activeChapter - 1); renderWorkbench(); showToast('章节已删除'); } }
    if (action === 'add-lesson') openLessonForm(item, activeChapter, null, renderWorkbench);
    if (action === 'edit-lesson') openLessonForm(item, Number(target.dataset.chapter), Number(target.dataset.index), renderWorkbench);
    if (action === 'delete-lesson') { if (window.confirm('确认删除该课时？')) { item.chapters[Number(target.dataset.chapter)].lessons.splice(Number(target.dataset.index), 1); renderWorkbench(); showToast('课时已删除'); } }
    if (action === 'resource') openResourcePicker(item, Number(target.dataset.chapter), Number(target.dataset.index), renderWorkbench);
    if (action === 'save') { item.status = '编排中'; item.updatedAt = '刚刚'; renderWorkbench(); showToast('编排草稿已保存'); }
    if (action === 'complete') { const complete = item.type !== '视频课程' || item.chapters.length > 0 && item.chapters.every(current => current.lessons.length > 0 && current.lessons.every(lesson => lesson.resources.some(resourceId => resources.find(resource => resource.id === resourceId)?.type === '教学视频'))); if (!complete) { showToast('视频课程每个课时都必须关联至少一个教学视频', 'error'); return; } if (!item.chapters.length || item.chapters.some(current => !current.lessons.length || current.lessons.some(lesson => !lesson.name || !lesson.target || !lesson.duration))) { showToast('请补齐章节和课时必填信息', 'error'); return; } item.status = '已完成'; item.updatedAt = '刚刚'; renderWorkbench(); renderContent(root()); showToast('课程编排已完成，可进入课程库'); }
  });
  renderWorkbench();
}
...
function handleClick(event) {
  ...
  if (action === 'publish-product') showToast('已带入视频商品发布流程');
  if (action === 'publish-class') showToast('已带入面授班级发布流程');
...
function deleteResource(id) {
  ...
  if (window.confirm(prompt)) { resources.splice(resources.indexOf(item), 1); renderResources(root()); showToast('资源已删除'); }
}
