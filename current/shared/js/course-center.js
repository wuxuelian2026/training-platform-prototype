...
const hoursText = (value) => (value ? `${value} 课时` : '—');
...
function courseFromApplication(item) {
  ...
  return { id: applicationCourseId(item), applicationId: item.id, source: '教师申报', name: item.name, type: item.type, major: item.major, teacher: item.teacher, hours: Number(item.hours) || 0, status: '待编排', updatedAt: demoTime(), chapters: [], difficulty: item.difficulty || '', ages: Array.isArray(item.ages) ? [...item.ages] : [] };
}
...
function libraryRow(item) {
  ...
  return `<tr><td><span class="primary-cell">${escapeHtml(key || '—')}</span></td><td><span class="primary-cell">${escapeHtml(item.name)}</span></td><td>${escapeHtml(item.source || '教师申报')}</td><td>${escapeHtml(item.type)}</td><td>${escapeHtml(item.major)}</td><td>${escapeHtml(item.teacher)}</td><td>${hoursText(item.hours)}</td><td>${statusCell}</td><td><button type="button" class="text-button course-version-link" data-action="library-versions" data-id="${item.id}">v${version}</button><span class="sub-cell">${escapeHtml(referenceLabel(references))}</span></td><td><div class="course-actions">${actions}</div></td></tr>`;
}
...
function openWorkbench(id) {
  ...
  const buildBlockers = () => {
    ...
    if (item.type !== '视频课程') {
      ...
      if (lessons.length !== Number(item.hours)) list.push({ chapterIndex: null, lessonIndex: null, text: `课时数需等于申报总课时 ${hoursText(item.hours)}，当前 ${lessons.length} 课时` });
    }
    return list;
  };
  const refreshMarks = () => {
