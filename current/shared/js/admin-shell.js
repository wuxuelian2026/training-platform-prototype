...
const demoRole = new URLSearchParams(window.location.search).get('role') || root?.dataset.role || 'academic_lead';
const roleLabels = { super_admin: '超级管理员', research_lead: '教研主管', academic_lead: '教务主管', course_consultant: '课程顾问', finance: '财务' };
...
const sidebar = navGroups.map(group => `<div class="nav-section"><div class="nav-section-title"><span class="nav-section-icon">${group.icon}</span><span>${group.label}</span></div>${group.items.filter(([, , , , allowedRoles]) => !allowedRoles || allowedRoles.includes(demoRole)).map(([icon, label, href, key]) => `<a class="nav-sublink ${href === currentPath || (!currentPath && key === active) ? 'active' : ''}" href="${href}"><span class="nav-icon">${icon}</span><span>${label}</span></a>`).join('')}</div>`).join('');
