...
function openClassForm(row = null, options = {}) {
  ...
  dialog.querySelector('#business-dialog-form')?.addEventListener('submit', event => {
    ...
    const record = { ...(row || {}), id: row?.id || demoId('class'), courseId: selected.id, source: selected.source || (selected.applicationId ? '教师申报' : '后台新增'), name: String(data.get('name')).trim(), className: String(data.get('name')).trim(), course: selected.name, courseName: selected.name, batchId: selectedBatch.id, batch: selectedBatch.name, category: selected.major.includes('中国') ? '舞蹈类' : selected.major.includes('声乐') ? '音乐类' : '美术类', professional: selected.major, price: price.toFixed(2), enrollStart, deadline, trialEnabled, trialFee, trialPrice: trialPrice ? trialPrice.toFixed(2) : '', trialNote: trialEnabled === '是' ? String(data.get('trialNote') || '').trim() : '', enrolled: row?.enrolled || 0, capacity, lessons: Number(selected.hours) || Number(row?.lessons) || 0, status: row?.id ? deriveClassStatus(row) : '待排班', display: row?.display || '未发布', scheduleStatus: row?.scheduleStatus || '未排班', scheduleVersion: Number(row?.scheduleVersion || 0), sessions: Array.isArray(row?.sessions) ? row.sessions : [], recommended: row?.recommended === true, created: row?.created || toLocalDateString() };
